...
function modify_showcode_name(){
    ...
    if(url.indexOf('show_code') >= 0){
        btn_showcode.innerHTML = "Hide Source Code"; // Translated "隐藏源码" to "Hide Source Code"
    }else{
        btn_showcode.innerHTML = "Show Source Code"; // Translated "显示源码" to "Show Source Code"
    }
}
...
function get_prompt(){
    $.ajax({
        ...
        Dialog.open(400,150,"Failed to fetch prompt!"); // Translated "获取提示失败！" to "Failed to fetch prompt!"
    });
}
...
function clean_upload_file(){
    $.ajax({
        ...
        Dialog.open(400,150,"Delete failed!"); // Translated "删除失败！" to "Delete failed!"
    });
}
...
$(function(){
    // Modify the "Show Source Code" button label
    modify_showcode_name();
    // Update copyright time
    update_copyright_time();
    // Set footer for bottom positioning
    setFooter();
    window.onresize = function(){
        setFooter();
    }

    // Highlight the selected menu item based on the current section
    var path = window.location.pathname;
    var pass_id = path.match(/Pass-\d{2}/i);
    $("#"+pass_id).addClass('a_is_selected');

    // Bind close event for pop-up dialog
